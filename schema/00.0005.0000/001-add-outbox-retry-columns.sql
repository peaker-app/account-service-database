CREATE TABLE IF NOT EXISTS `__EFMigrationsHistory` (
    `MigrationId` varchar(150) NOT NULL,
    `ProductVersion` varchar(32) NOT NULL,
    PRIMARY KEY (`MigrationId`)
);

START TRANSACTION;
IF NOT EXISTS(SELECT * FROM `__EFMigrationsHistory` WHERE `MigrationId` = '20260724100852_InitialAccountSchema')
BEGIN
    CREATE TABLE `outbox_messages` (
        `id` char(36) NOT NULL,
        `type` varchar(500) NOT NULL,
        `content` longtext NOT NULL,
        `occurred_at_utc` datetime(6) NOT NULL,
        `processed_at_utc` datetime(6) NULL,
        `error` longtext NULL,
        PRIMARY KEY (`id`)
    );
END;

IF NOT EXISTS(SELECT * FROM `__EFMigrationsHistory` WHERE `MigrationId` = '20260724100852_InitialAccountSchema')
BEGIN
    CREATE TABLE `processed_messages` (
        `message_id` char(36) NOT NULL,
        `processed_at_utc` datetime(6) NOT NULL,
        PRIMARY KEY (`message_id`)
    );
END;

IF NOT EXISTS(SELECT * FROM `__EFMigrationsHistory` WHERE `MigrationId` = '20260724100852_InitialAccountSchema')
BEGIN
    CREATE TABLE `profiles` (
        `id` char(36) NOT NULL,
        `user_id` char(36) NOT NULL,
        `display_name` varchar(60) NOT NULL,
        `slug` varchar(80) NOT NULL,
        `bio` varchar(500) NULL,
        `avatar_public_id` varchar(255) NULL,
        `avatar_url` varchar(500) NULL,
        `country_code` varchar(2) NULL,
        `visibility` varchar(20) NOT NULL,
        `created_at_utc` datetime(6) NOT NULL,
        `updated_at_utc` datetime(6) NOT NULL,
        PRIMARY KEY (`id`)
    );
END;

IF NOT EXISTS(SELECT * FROM `__EFMigrationsHistory` WHERE `MigrationId` = '20260724100852_InitialAccountSchema')
BEGIN
    CREATE TABLE `profile_stats` (
        `profile_id` char(36) NOT NULL,
        `total_ascents` int NOT NULL,
        `distinct_peaks` int NOT NULL,
        `highest_altitude_m` int NOT NULL,
        `highest_peak_id` char(36) NULL,
        `highest_peak_name` varchar(200) NULL,
        `last_ascent_date` date NULL,
        `updated_at_utc` datetime(6) NOT NULL,
        PRIMARY KEY (`profile_id`),
        CONSTRAINT `FK_profile_stats_profiles_profile_id` FOREIGN KEY (`profile_id`) REFERENCES `profiles` (`id`) ON DELETE CASCADE
    );
END;

IF NOT EXISTS(SELECT * FROM `__EFMigrationsHistory` WHERE `MigrationId` = '20260724100852_InitialAccountSchema')
BEGIN
    CREATE INDEX `ix_outbox_messages_processed_at_utc` ON `outbox_messages` (`processed_at_utc`);
END;

IF NOT EXISTS(SELECT * FROM `__EFMigrationsHistory` WHERE `MigrationId` = '20260724100852_InitialAccountSchema')
BEGIN
    CREATE UNIQUE INDEX `ux_profiles_slug` ON `profiles` (`slug`);
END;

IF NOT EXISTS(SELECT * FROM `__EFMigrationsHistory` WHERE `MigrationId` = '20260724100852_InitialAccountSchema')
BEGIN
    CREATE UNIQUE INDEX `ux_profiles_user_id` ON `profiles` (`user_id`);
END;

IF NOT EXISTS(SELECT * FROM `__EFMigrationsHistory` WHERE `MigrationId` = '20260724100852_InitialAccountSchema')
BEGIN
    INSERT INTO `__EFMigrationsHistory` (`MigrationId`, `ProductVersion`)
    VALUES ('20260724100852_InitialAccountSchema', '10.0.10');
END;

IF NOT EXISTS(SELECT * FROM `__EFMigrationsHistory` WHERE `MigrationId` = '20260725133525_AddProfileAscents')
BEGIN
    ALTER TABLE `profile_stats` ADD `public_distinct_peaks` int NOT NULL DEFAULT 0;
END;

IF NOT EXISTS(SELECT * FROM `__EFMigrationsHistory` WHERE `MigrationId` = '20260725133525_AddProfileAscents')
BEGIN
    ALTER TABLE `profile_stats` ADD `public_highest_altitude_m` int NOT NULL DEFAULT 0;
END;

IF NOT EXISTS(SELECT * FROM `__EFMigrationsHistory` WHERE `MigrationId` = '20260725133525_AddProfileAscents')
BEGIN
    ALTER TABLE `profile_stats` ADD `public_highest_peak_id` char(36) NULL;
END;

IF NOT EXISTS(SELECT * FROM `__EFMigrationsHistory` WHERE `MigrationId` = '20260725133525_AddProfileAscents')
BEGIN
    ALTER TABLE `profile_stats` ADD `public_highest_peak_name` varchar(200) NULL;
END;

IF NOT EXISTS(SELECT * FROM `__EFMigrationsHistory` WHERE `MigrationId` = '20260725133525_AddProfileAscents')
BEGIN
    ALTER TABLE `profile_stats` ADD `public_last_ascent_date` date NULL;
END;

IF NOT EXISTS(SELECT * FROM `__EFMigrationsHistory` WHERE `MigrationId` = '20260725133525_AddProfileAscents')
BEGIN
    ALTER TABLE `profile_stats` ADD `public_total_ascents` int NOT NULL DEFAULT 0;
END;

IF NOT EXISTS(SELECT * FROM `__EFMigrationsHistory` WHERE `MigrationId` = '20260725133525_AddProfileAscents')
BEGIN
    CREATE TABLE `profile_ascents` (
        `id` char(36) NOT NULL,
        `profile_id` char(36) NOT NULL,
        `ascent_id` char(36) NOT NULL,
        `peak_id` char(36) NOT NULL,
        `peak_name` varchar(200) NOT NULL,
        `peak_altitude_m` int NOT NULL,
        `ascent_date` date NOT NULL,
        `visibility` varchar(20) NOT NULL,
        `created_at_utc` datetime(6) NOT NULL,
        `updated_at_utc` datetime(6) NOT NULL,
        PRIMARY KEY (`id`),
        CONSTRAINT `FK_profile_ascents_profiles_profile_id` FOREIGN KEY (`profile_id`) REFERENCES `profiles` (`id`) ON DELETE CASCADE
    );
END;

IF NOT EXISTS(SELECT * FROM `__EFMigrationsHistory` WHERE `MigrationId` = '20260725133525_AddProfileAscents')
BEGIN
    CREATE INDEX `ix_profile_ascents_peak` ON `profile_ascents` (`peak_id`);
END;

IF NOT EXISTS(SELECT * FROM `__EFMigrationsHistory` WHERE `MigrationId` = '20260725133525_AddProfileAscents')
BEGIN
    CREATE INDEX `ix_profile_ascents_profile` ON `profile_ascents` (`profile_id`);
END;

IF NOT EXISTS(SELECT * FROM `__EFMigrationsHistory` WHERE `MigrationId` = '20260725133525_AddProfileAscents')
BEGIN
    CREATE UNIQUE INDEX `ux_profile_ascents_ascent` ON `profile_ascents` (`ascent_id`);
END;

IF NOT EXISTS(SELECT * FROM `__EFMigrationsHistory` WHERE `MigrationId` = '20260725133525_AddProfileAscents')
BEGIN
    INSERT INTO `__EFMigrationsHistory` (`MigrationId`, `ProductVersion`)
    VALUES ('20260725133525_AddProfileAscents', '10.0.10');
END;

IF NOT EXISTS(SELECT * FROM `__EFMigrationsHistory` WHERE `MigrationId` = '20260727095755_AddCollections')
BEGIN
    CREATE TABLE `collections` (
        `id` char(36) NOT NULL,
        `profile_id` char(36) NOT NULL,
        `kind` varchar(20) NOT NULL,
        `name` varchar(60) NOT NULL,
        `description` varchar(500) NULL,
        `created_at_utc` datetime(6) NOT NULL,
        `updated_at_utc` datetime(6) NOT NULL,
        PRIMARY KEY (`id`),
        CONSTRAINT `FK_collections_profiles_profile_id` FOREIGN KEY (`profile_id`) REFERENCES `profiles` (`id`) ON DELETE CASCADE
    );
END;

IF NOT EXISTS(SELECT * FROM `__EFMigrationsHistory` WHERE `MigrationId` = '20260727095755_AddCollections')
BEGIN
    CREATE TABLE `collection_peaks` (
        `id` char(36) NOT NULL,
        `peak_id` char(36) NOT NULL,
        `peak_name` varchar(200) NOT NULL,
        `peak_altitude_m` int NOT NULL,
        `added_at_utc` datetime(6) NOT NULL,
        `collection_id` char(36) NOT NULL,
        PRIMARY KEY (`id`),
        CONSTRAINT `FK_collection_peaks_collections_collection_id` FOREIGN KEY (`collection_id`) REFERENCES `collections` (`id`) ON DELETE CASCADE
    );
END;

IF NOT EXISTS(SELECT * FROM `__EFMigrationsHistory` WHERE `MigrationId` = '20260727095755_AddCollections')
BEGIN
    ALTER TABLE `collections` MODIFY `name` VARCHAR(60) CHARACTER SET utf8mb4 COLLATE utf8mb4_0900_ai_ci NOT NULL;
END;

IF NOT EXISTS(SELECT * FROM `__EFMigrationsHistory` WHERE `MigrationId` = '20260727095755_AddCollections')
BEGIN
    CREATE INDEX `ix_collection_peaks_peak` ON `collection_peaks` (`peak_id`);
END;

IF NOT EXISTS(SELECT * FROM `__EFMigrationsHistory` WHERE `MigrationId` = '20260727095755_AddCollections')
BEGIN
    CREATE UNIQUE INDEX `ux_collection_peaks_unique` ON `collection_peaks` (`collection_id`, `peak_id`);
END;

IF NOT EXISTS(SELECT * FROM `__EFMigrationsHistory` WHERE `MigrationId` = '20260727095755_AddCollections')
BEGIN
    CREATE UNIQUE INDEX `ux_collections_profile_name` ON `collections` (`profile_id`, `name`);
END;

IF NOT EXISTS(SELECT * FROM `__EFMigrationsHistory` WHERE `MigrationId` = '20260727095755_AddCollections')
BEGIN
    ALTER TABLE `collections` ADD COLUMN `default_slot` TINYINT GENERATED ALWAYS AS (CASE WHEN `kind` = 'WantToClimb' THEN 1 END) STORED;
END;

IF NOT EXISTS(SELECT * FROM `__EFMigrationsHistory` WHERE `MigrationId` = '20260727095755_AddCollections')
BEGIN
    CREATE UNIQUE INDEX `ux_collections_profile_default` ON `collections` (`profile_id`, `default_slot`);
END;

IF NOT EXISTS(SELECT * FROM `__EFMigrationsHistory` WHERE `MigrationId` = '20260727095755_AddCollections')
BEGIN
    INSERT INTO `__EFMigrationsHistory` (`MigrationId`, `ProductVersion`)
    VALUES ('20260727095755_AddCollections', '10.0.10');
END;

IF NOT EXISTS(SELECT * FROM `__EFMigrationsHistory` WHERE `MigrationId` = '20260810091049_RemoveAvatarUrl')
BEGIN
    ALTER TABLE `profiles` DROP COLUMN `avatar_url`;
END;

IF NOT EXISTS(SELECT * FROM `__EFMigrationsHistory` WHERE `MigrationId` = '20260810091049_RemoveAvatarUrl')
BEGIN
    INSERT INTO `__EFMigrationsHistory` (`MigrationId`, `ProductVersion`)
    VALUES ('20260810091049_RemoveAvatarUrl', '10.0.10');
END;

IF NOT EXISTS(SELECT * FROM `__EFMigrationsHistory` WHERE `MigrationId` = '20260813112928_AddOutboxRetryColumns')
BEGIN
    DROP INDEX ix_outbox_messages_processed_at_utc ON outbox_messages;
END;

IF NOT EXISTS(SELECT * FROM `__EFMigrationsHistory` WHERE `MigrationId` = '20260813112928_AddOutboxRetryColumns')
BEGIN
    ALTER TABLE `outbox_messages` ADD `attempt_count` int NOT NULL DEFAULT 0;
END;

IF NOT EXISTS(SELECT * FROM `__EFMigrationsHistory` WHERE `MigrationId` = '20260813112928_AddOutboxRetryColumns')
BEGIN
    ALTER TABLE `outbox_messages` ADD `next_attempt_at_utc` datetime(6) NULL;
END;

IF NOT EXISTS(SELECT * FROM `__EFMigrationsHistory` WHERE `MigrationId` = '20260813112928_AddOutboxRetryColumns')
BEGIN
    CREATE INDEX `ix_outbox_messages_processed_at_utc` ON `outbox_messages` (`processed_at_utc`, `next_attempt_at_utc`);
END;

IF NOT EXISTS(SELECT * FROM `__EFMigrationsHistory` WHERE `MigrationId` = '20260813112928_AddOutboxRetryColumns')
BEGIN
    INSERT INTO `__EFMigrationsHistory` (`MigrationId`, `ProductVersion`)
    VALUES ('20260813112928_AddOutboxRetryColumns', '10.0.10');
END;

COMMIT;

